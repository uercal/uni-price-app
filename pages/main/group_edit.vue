	...
	<view>		
		<form>			
			...
			<view class="cu-list menu sm-border card-menu margin-top'">				
				<view class="cu-item">
					<view class="content">						
						<text class="text-black" style="width: 30%;">商品名称</text>
						<text style="width:25%;" class="text-black">原价</text>
						<text style="width:25%;" class="text-black">现价</text>
				...
				<view class="cu-item" v-for="(item,index) in goods_price" :key="item.goods_id">
					<view class="content">
						<text class="text-grey" style="width: 30%;">{{item.goods_name}}</text>
						<text style="width:25%;" class="text-grey">{{item.price}}元</text>
						<text style="width:25%;" class="text-grey" v-html="(newPrice[index].price)+'元'"></text>
						<view style="width:20%;display: flex;align-items: center;">
	...
	export default {
		...
		methods: {				
			...
			getInfo(){
				...
				Service._post('Member/getGroupInfo',{group_id:group_id},function(res){
					_this.group_name = res.data.group_name;
					let data =res.data.data;
					// 
					data.map(e=>{						
						newPrice.map(_e=>{
							if(_e.goods_id==e.goods_id){								
								_e.rate = e.rate;	
								let price = e.rate * _e.price /100;
								_e.price = _this.tofixNumber(price);
							}
						})
					});
				})
